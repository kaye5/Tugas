...
public class TestInventory {
	public static void main(String []args) {
		...
		if (inv.getItemTotalCount() != 10) {
			System.out.println("total count error1");
		}
		if (inv.getItemCount(items[0]) != 5) {
			System.out.println("Item count error2");
		}
                
		if (inv.getItemCount(items[3]) != -1) {
			System.out.println("Item count error3");
		}
                
		if (inv.takeItem(items[0], 1) != 1) {
			System.out.println("take item error4");
		}
		if (inv.takeItem(items[0], 10) != 4) {
			System.out.println("take item error5");
		}

		try {
			inv.takeItem(items[3], 1);
			System.out.println("take item error6");
		} catch (NoSuchElementException e) {
		}

		try {
			inv.takeItem(items[1], -1);
			System.out.println("take item error7");
